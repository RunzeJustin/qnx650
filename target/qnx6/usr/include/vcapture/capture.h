 ...
 *
 * Returns:
 *        a pointer to a new context, or
 *        a NULL if an error occurred, errno is set.
 * errno:
 *        EINVAL -- invalid flags.
 *        ENOMEM -- unable to allocate memory.
 *        <list of mmap_device_io()/ChannelCreate()/pthread_create().. errors>
 */
...
it to be valid.  In worst-case hardware the buffer is at a hard-coded physical
 ...
 * The existing buffers that were allocated before this call are freed immediately.
 ...
 *         ENOMEM -- unable to allocate memory.
 *         ENOTSUP -- this driver doesn't support buffer allocation.
 *
 * Example:
 *  ...
 ...
 *
 * NOTE: There is only one operational context per device/source combination.
 * When attempting to use the same device/source combination, the first context to call capture_update() is one
 * to be used when capture_get_frame(). The other(s) will fail on capture_update().
 *
 ...
 *        ENODEV -- no such capturing device/source (id).
 *        EBUSY  -- the context is already in use.
 *
 ...
 *  EIO       -- h/w input/output error
 *  ENOMEM    -- capturing is not possible, as ALL capture buffers are locked by the user (not released
...
#define CAPTURE_PROPERTY_INVERT_FID_POL		CAPTURE_PROPERTY( 'Q', 'L', 'F', 'I' )
	/* R/W		int			Controls whether the field ID signal is inverted (0 or 1; or -1 for default). */

#define CAPTURE_PROPERTY_INVERT_VSYNC_POL	CAPTURE_PROPERTY( 'Q', 'L', 'H', 'S' )
	/* R/W		int			Controls whether the vsync polarity is inverted (0 or 1; or -1 for default). */

#define CAPTURE_PROPERTY_INVERT_HSYNC_POL	CAPTURE_PROPERTY( 'Q', 'L', 'V', 'S' )
	/* R/W		int			Controls whether the hsync polarity is inverted (0 or 1; or -1 for default). */

#define CAPTURE_PROPERTY_INVERT_CLOCK_POL	CAPTURE_PROPERTY( 'Q', 'L', 'P', 'C' )
	/* R/W		int			Controls whether the clock polarity is inverted (0 or 1; or -1 for default). */

#define CAPTURE_PROPERTY_INVERT_DATAEN_POL	CAPTURE_PROPERTY( 'Q', 'L', 'D', 'E' )
	/* R/W		int			Controls whether the data_en polarity is inverted (0 or 1; or -1 for default). */

#define CAPTURE_PROPERTY_INVERT_DATA_POL CAPTURE_PROPERTY( 'Q', 'L', 'D', 'A' )
	/* R/W		int			Controls whether the data input polarity is inverted (0 or 1; or -1 for default). */
...
/** NOTE: A capture driver provides a list of supported devices (NDEVICES),
...
#define CAPTURE_PROPERTY_PLANAR_OFFSETS		CAPTURE_PROPERTY( 'Q', 'P', 'L', 'O' )
	/* R/W		[3] int32_t		Offset from the base address for each of the Y, U, and V
	 * 					components of planar YUV formats. */

#define CAPTURE_PROPERTY_THREAD_PRIORITY	CAPTURE_PROPERTY( 'Q', 'T', 'P', 'R' )
	/* R/W		int			The scheduling priority of the capture thread.
	 *					The default value is "application priority + 20". */
...
#define CAPTURE_PROPERTY_DATA_BUS_WIDTH		CAPTURE_PROPERTY( 'Q', 'D', 'B', 'W' )
	/* R/W		int			The data bus width, for parallel interface. Possible values: 8, 10, 16... */
...
#define CAPTURE_PROPERTY_VERBOSITY		CAPTURE_PROPERTY( 'Q', 'V', 'B', 'R' )
	/* R/W		uint32_t		The verbose level when capturing. Possible values: 0 (default), 1, 2... */
...
main() {
	...
	if( !capture_is_property( context, CAPTURE_PROPERTY_FRAME_BUFFERS )
	 || !capture_is_property( context, CAPTURE_PROPERTY_DST_FORMAT )
	...
	capture_set_property_pv( context, CAPTURE_PROPERTY_FRAME_BUFFERS, n_pointers, pointers );
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/release/service-screen/branches/moonraker/trunk/lib/capture/public/vcapture/capture.h $ $Rev: 734989 $")
#endif
