 ...
 *
 * @brief Functions that are to be implemented by the mtouch driver and are called
 * by libinputevents to fetch the data required to transform the device
...
/** The height when @c MTOUCH_CAPABILITIES_HEIGHT isn't set, or if this capability
 *  isn't available.
 */
...
/** The pressure when @c MTOUCH_CAPABILITIES_PRESSURE isn't set, or if
 *  this capability isn't available.
 */
...
/** A multi-touch device flag. Devices that don't have contact IDs assigned as
...
typedef struct {
	...
	*                 specified @c digit_idx. @c contact_id is updated by this
	*                 function upon its successful execution.
	...
	*            @c valid is updated by this function upon its successful
	*            execution.
	* - @c arg User information.
	*
	* @b Returns
	...
	*        specified @c digit_idx. @c x is updated by this function upon its
	*        successful execution.
	* - @c y Pointer to the y coordinate of the touch-related event for the
	*        specified @c digit_idx. @c y is updated by this function upon its
	*        successful execution.
	* - @c arg User information.
	*
	* @b Returns
	...
	*                 with the screen. @c down_count is updated by this function
	...
	*                  specified @c digit_idx. @c touch_width is updated by this
	*                  function upon its successful execution.
	* - @c arg User information.
	*
	* @b Returns
	...
	*                   specified @c digit_idx. @c touch_height is updated by
	*                   this function upon its successful execution.
	* - @c arg User information.
	*
	* @b Returns
	* - @c 0 if successful, otherwise non-zero if an error occurred.
	...
	* @e (*get_touch_width)() and @e (*get_touch_height)() callback functions.
	*
	* @b Arguments
	* - @c packet Data packet that contains information on the touch-related event.
	*             This data packet is the same packet that was passed into
	...
	*                     for the specified @c digit_idx. @c touch_pressure is
	*                     updated by this function upon its successful execution.
	* - @c arg User information.
	*
	* @b Returns
	* - @c 0 if successful, otherwise non-zero if an error occurred.
	...
	*             requesting. @c seq_id is updated by this function upon its
	*             successful execution.
	* - @c arg User information.
	*
	* @b Returns
	...
	*                   @c #contact_type_e. @c contact_type is updated by this
	*                   function upon its successful execution.
	* - @c arg User information.
	*
	* @b Returns
...
*   callback functions to the Input Events frameowrk. @c params and @c funcs
*   are required to be configured before passing them as arguments to this
...
*   @return Nothing.
*/
void mtouch_driver_process_packet(struct mtouch_device* device, void* packet, void* arg);
